"""Command-line interface for Quantum Vaults.

This module provides a Click-based CLI that mirrors the functionality of the
existing GUI application without modifying the underlying logic. Users can list
connected clients and generate vaults with a selected peer through the
broker-managed network.
"""
...
@click.group()
def cli() -> None:
    """Quantum Vaults command-line utilities."""
...
@click.option("--broker-port", default=CONFIG["broker"]["port"], type=int,
              show_default=True, help="Broker port")
@click.option("--name", required=True, help="Client name")
def run(peer: str, broker_host: str, broker_port: int, name: str) -> None:
    """Generate a vault with the selected peer."""
    ...
    sender.add_file(peer, location, dest_location)
