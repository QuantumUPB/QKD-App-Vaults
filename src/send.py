...
class SendFile:
    def __init__(self, to_name, src_location, dest_location):
        ...
        message = ""
        file_name = f"{to_name}_{time.strftime('%Y%m%d-%H%M%S')}.csv"
        print("Sending: ")
...
class FileSendWorker(QThread):
    ...
    def add_file(self, to_name, src_location, dest_location):
        sending_file = SendFile(to_name, src_location, dest_location)
    ...
    def send_metadata(self, sending_file):
        ...
        try:
            ...
            encrypted_message_bytes = bytes(str(sending_file.total_segments) + "/" + sending_file.file_name + "/" + sending_file.src_location, 'utf-8')
    ...
    def send_segment(self, sending_file):
        ...
        print(sending_file.nr_segments)
        ...
        key_ids_list = []
        keys_list = []
        while len(key_ids_list) < seg_length:
        ...
        # print(key_ids_list)
        sending_file.save_keys_to_file(keys_list, key_ids_list)

        # batch_len = min(seg_length, len(message))
        batch_len = seg_length
